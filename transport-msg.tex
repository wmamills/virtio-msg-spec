...
Drivers use \msgref{EVENT_AVAIL} to notify the device of available buffers. A
bus implementation may use out-of-band mechanisms as wakeup hints, but
\msgref{EVENT_AVAIL} remains the authoritative transport message for each
driver notification occurrence.
...
\begin{itemize}
  \item A driver MUST use \msgref{EVENT_AVAIL} to notify the device of available
        buffers.
...
\begin{itemize}
  \item For each driver notification occurrence, a bus implementation MUST
        deliver exactly one authoritative \msgref{EVENT_AVAIL} message to the
        device side.
  \item A bus implementation MAY use out-of-band notification signals as wakeup
        hints, but those hints MUST NOT create an additional semantic
        notification occurrence.
  \item A bus implementation MUST use a bounded delivery policy for
        \msgref{EVENT_AVAIL}. If delivery cannot be completed within that
        policy, the bus implementation MUST surface a transport-visible failure
        and MUST trigger recovery according to the reset and status semantics in
        \ref{sec:Virtio Transport Options / Virtio Over Messages / Device Operation}.
\end{itemize}
...
from the device (or device-side bus) to the driver. A bus implementation may
use other signals such as interrupts or polling as wakeup hints. For each
notification occurrence, the bus instance selects one authoritative
transport-message path: forward the originating in-band message, or synthesize
one message from bus-observed signaling.
...
\begin{itemize}
  \item A device (or device-side bus) MUST use \msgref{EVENT_CONFIG} to notify
        the driver of configuration changes or status updates.
  \item A device MUST follow the active configuration semantics profile in
        \ref{sec:Virtio Transport Options / Virtio Over Messages / Basic Concepts / Configuration Semantics Profiles}
        for \msgref{EVENT_CONFIG} generation and offset/length handling.
  \item A device MUST use \msgref{EVENT_USED} to notify the driver of used
        buffers on a virtqueue, unless the device relies on polling or another
        agreed-upon completion mechanism.
\end{itemize}
...
\begin{itemize}
  ...
  \item A driver MUST follow the active configuration semantics profile in
        \ref{sec:Virtio Transport Options / Virtio Over Messages / Basic Concepts / Configuration Semantics Profiles}
        for \msgref{EVENT_CONFIG} generation and offset/length interpretation.
...
\begin{itemize}
  \item For each \msgref{EVENT_CONFIG} or \msgref{EVENT_USED} notification
        occurrence, a bus implementation MUST deliver exactly one
        transport-message occurrence to the driver.
  \item A bus implementation MUST choose exactly one authoritative path per bus
        instance and notification type: forwarding of originating in-band
        messages, or synthesis from bus-observed signaling. It MUST NOT use both
        paths for the same notification occurrence.
  \item Out-of-band signals used with device notifications MUST be treated as
        wakeup hints only and MUST NOT create additional semantic notification
        occurrences.
  \item A bus implementation MUST use a bounded notification-delivery policy.
        If delivery cannot be completed within that policy, it MUST surface a
        transport-visible failure to the driver side and MUST trigger recovery
        using reset and status semantics.
\end{itemize}
...
\begin{itemize}
  ...
  \item For each runtime notification occurrence, a bus implementation MUST
        produce exactly one authoritative transport-message occurrence and MUST
        NOT combine forwarding and synthesis for the same occurrence.
  ...
  \item A bus implementation MUST use bounded timeout and polling policies for
        runtime-notification delivery and MUST define recovery behavior when the
        bounded policy is exhausted.
\end{itemize}
...
\begin{itemize}
  \item A device MUST generate one used-buffer notification occurrence when it
        processes buffers on a virtqueue unless the device relies on polling or
        another agreed-upon completion mechanism.
  \item If the bus instance uses forwarding as the authoritative delivery path,
        the device MUST send \msgref{EVENT_USED} for that notification
        occurrence.
\end{itemize}
