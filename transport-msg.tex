...
\begin{itemize}
  \item \textbf{Bus Instances and Devices}: Each bus instance can advertise
        ...
        Every discovered device is assigned a \emph{device ID}.
...
the bus instance’s \textbf{maximum message size} limit.
...
support receiving it, the device \emph{MAY} rely on standard virtqueue polling
mechanisms to detect virtqueue changes.
...
\begin{itemize}
  ...
  \item If a device does not suppport to send \msgref{EVENT_CONFIG}, the driver
...
an \msgref{EVENT_CONFIG} with a \emph{device status} indicating an error state,
...
suppported, verifying the target device ID exists).
...
\begin{itemize}
  ...
  \item A driver \textbf{MUST} provide the correct \emph{Configuration
      Generation Count} in \msgref{SET_CONFIG}, or else it \emph{MUST} handle
    an \msgref{ERROR}.
