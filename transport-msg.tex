...
\begin{itemize}
  ...
  \item \textbf{Number of Blocks}: How many blocks the driver wishes to retrieve
        or modify in a single message.  This is explicit in GET_FEATURES and
        inferred from msg_size in SET_FEATURES.
  \item \textbf{Feature Data}: The 32 bit values representing a device's
...
\begin{itemize}
  ...
  \item \field{dev_num}:
     ...
     Transport Message and must be zero for Bus messages.
  \item \field{msg_size}:
    Indicates the total length of the message payload including the header.
  \item \field{req_id}:
    The req_id is echoed back in the response with out change or interpretation.
    The request originator \emph{MAY} make this field unique and use it to match
    responses to concurrent requests.
\end{itemize}
...
If the generation count matches the device's current value, the device
will update its configuration and echo back the new value of configuration data.
The new data may be different than what was sent in the request.

If the generation count does not match the device's current value, the device
\emph{MUST} reject the update and return no configuration data.
...
& 12 & 4 & Number of bytes (\emph{MAY} be zero) \\
& 16 & ... & Configuration data \\
\hline
\end{tabular}

The number of bytes field \emph{MAY} be zero. If so the configuration data field
...
\begin{itemize}
  \item Implementations \emph{MAY} use \busref{GET_DEVICES}
