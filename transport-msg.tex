...
The maximum messgae size is from the common transport level point of
view and inculdes the headers and payload described here.  If the bus adds it
own overhead (e.x. its own header) this is not included in the maximum message
size. The maximum useful message size is currentlty expected to be 274.
This value is large enough to support a GET_CONFIG or SET_CONFIG message with a
config payload of 256 bytes.
...
\begin{description}
  \item[\textbf{Bus Messages}:]
    ...
    \busref{DEVICE_REMOVED}), or assesing bus-wide health (\busref{PING}).
...
The driver \emph{SHOULD} treat this situation as if \msgref{SET_DEVICE_STATUS}(0) was
