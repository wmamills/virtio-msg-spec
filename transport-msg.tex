...
then calling \msgref{SET_VQUEUE} with the chosen size and physical
addresses.
...
\begin{itemize}
  ...
  \item A driver MUST ensure the queue size provided in \msgref{SET_VQUEUE} does
        ...
        descriptor/driver/device physical addresses before enabling the queue.
...
\begin{itemize}
  \item A device MUST report accurate maximum queue sizes in \msgref{GET_VQUEUE}
        ...
        descriptor, driver, and device physical addresses).
...
driver, and device area physical addresses.
...
\msgref{SET_VQUEUE} programs a virtqueue's size and physical addresses. The
driver selects a queue index, supplies the desired size (not exceeding the
maximum reported via \msgref{GET_VQUEUE}), and provides physical addresses for
the descriptor, driver, and device areas.
