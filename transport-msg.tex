...
\begin{itemize}
  ...
  space, virtqueues), so existing virtio drivers and device backends can
...
\begin{itemize}
  \item \textbf{Bus Instances and Devices}: Each bus instance can advertise
        different capabilities (e.g., transport revision, max message size) and
        may discover devices via a message-based or external mechanism. Every
        discovered device is assigned a \emph{device ID}.
  \item \textbf{Driver (Frontend)}: Communicates with the bus to learn about
        ...
        and virtqueue setup.
  \item \textbf{Device Backends}: Implement virtio device functionality and
        respond to the transport messages. The bus forwards these messages to
        the correct backend based on device ID.
\end{itemize}

\subsubsection{Optional Bus Messages}

This specification \emph{defines} messages such as \busref{GET_DEVICES},
\busref{DEVICE_ADDED}, \busref{DEVICE_REMOVED}, and \busref{STATUS} for a
completely message-based approach to enumeration, hotplug, and bus-wide resets.
...
\begin{itemize}
  ...
  \item A \textbf{maximum message size} (\texttt{max\_size}) representing the
        largest payload (in bytes) the bus can reliably carry per request or
        response.
\end{itemize}

These parameters \emph{MAY} vary between bus instances within the same system.
The driver obtains a bus’s revision and \texttt{max\_size} through an
\emph{implementation-defined} mechanism, which could be:
\begin{itemize}
...
message exceeding \texttt{max\_size}, and it \emph{MUST} avoid using advanced
features or messages unavailable in the bus’s advertised revision.
...
\hline
\field{revision} & \field{max_size} & \field{features}      & remarks \\
\hline \hline
...
\subsubsection{Device IDs and Enumeration}
\label{sec:Virtio Transport Options / Virtio Over Messages / Basic Concepts / DeviceIDs}

Each virtio-msg bus instance contains one or more \emph{devices}, identified
by a 16-bit \textbf{device ID}. Buses \emph{MAY} discover these device IDs
in multiple ways:
\begin{itemize}
  \item \textbf{Message-Based Enumeration}: Using \busref{GET_DEVICES} to query
        which IDs exist (optional).
  \item \textbf{Platform Data}: A device tree, ACPI tables, or hypervisor calls
        might inform the bus of available device IDs and their properties.
\end{itemize}

Once a bus confirms that a given device ID is valid (regardless of the method),
it typically issues \msgref{GET_DEVICE_INFO} to retrieve the device’s Virtio ID
...
inconsistent updates. This count is incremented whenever the device backend
changes its own configuration internally. The driver learns the current count
via \msgref{GET_CONFIG} and \emph{MUST} include it in \msgref{SET_CONFIG}
requests.

\begin{itemize}
  \item If the driver’s generation count does not match the device’s current
        count, the \msgref{SET_CONFIG} request \emph{MUST} be rejected (typically
        via \msgref{ERROR}).
  \item The driver then \emph{SHOULD} re-read the configuration (e.g., by
        \msgref{GET_CONFIG}), retrieve the updated count, and attempt the update
        again if necessary.
\end{itemize}
...
\textbf{256-bits blocks}, accessed via \msgref{GET_FEATURES} and
\msgref{SET_FEATURES}. Each block corresponds to up to 256 features:

\begin{itemize}
  \item \textbf{Block Index}: Identifies the starting block (e.g., block 0 for
        features 0--255, block 1 for features 256--511, etc.).
  \item \textbf{Number of Blocks}: How many blocks the driver wishes to retrieve
        or modify in a single message.
  \item \textbf{Feature Data}: The 256 bits values representing a device’s
        supported or requested feature bits.
\end{itemize}

A device \emph{MUST} respond with \msgref{ERROR} if the driver requests
blocks beyond those the device implements, or if the request size exceeds
the bus instance’s \texttt{max\_size} limit.
...
\begin{description}
  \item[\textbf{Bus Messages}:]
    Intended for global bus operations such as enumerating device IDs
    (\busref{GET_DEVICES}), managing device hotplug events (\busref{DEVICE_ADDED},
    \busref{DEVICE_REMOVED}), or sending a bus-wide reset or suspend signal
    (\busref{STATUS}). These messages are \emph{optional} in environments where
  ...
  \item[\textbf{Transport Messages}:]
    ...
    \begin{itemize}
      ...
      \item Handling device status and notifications (\msgref{SET_DEVICE_STATUS},
        \msgref{EVENT_USED}, etc.).
    \end{itemize}
    The bus \emph{MUST} relay these messages to the correct device backend,
...
\begin{itemize}
  \item \field{type}:
    \begin{itemize}
      ...
      \item Bit[1]: Identifies if a message is a Transport Message (0) or a
          Bus Message (1).
      \item Bit[2-7] Are reserverd for future use and must be zero.
    \end{itemize}
  \item \field{msg_id}:
  ...
  \item \field{dev_id}:
     Identifies the Device ID the message is targetting or is coming from for
...
\begin{itemize}
  ...
  \item By using \busref{GET_DEVICES} messages (defined in
    ...
    to query which device IDs are present on this bus instance.
\end{itemize}
...
\subsubsection{Resetting the Bus}
\label{sec:Virtio Transport Options / Virtio Over Messages / Bus Operation / Resetting the Bus}

A bus \emph{MAY} encounter conditions that require a global reset of all attached
devices (e.g., major error or system suspend/resume). If the bus implements a
message-based reset mechanism, it \emph{MAY} send or receive \busref{STATUS}
messages carrying a \emph{Reset} state. However, the bus \emph{MAY} also handle
resets through other platform-defined means (e.g., firmware calls, hardware
interrupt signals) without using \busref{STATUS} at all.

When a global reset occurs—regardless of how it is signaled—the bus
\emph{SHOULD}:
\begin{itemize}
  \item Treat previously enumerated devices as uninitialized and no longer
        available,
  \item Discard or invalidate pending I/O requests at the bus level,
  \item Revert to a known baseline state so that new device discovery can start
        cleanly.
\end{itemize}
Drivers \emph{SHOULD} be prepared to re-enumerate devices after a bus reset if
those devices remain valid.
...
backend). Implementations \emph{MAY} use other keepalive or health-check
...
messages} to the correct backend for each device ID. Typically, the bus does
...
\begin{enumerate}
  \item \textbf{Obtain Device Information:}  
    The driver queries the device’s ID, vendor ID, feature block count, and
    configuration size using \msgref{GET_DEVICE_INFO}.
  \item \textbf{Negotiate Features:}  
    The driver retrieves the device’s feature bits (\msgref{GET_FEATURES}),
    determines which features it wants to enable, and writes back any updated
    bits (\msgref{SET_FEATURES}).
...
Once the bus or driver knows a device ID is present, it \emph{SHOULD} send a
...
\begin{itemize}
  ...
  \item \textbf{Number of Feature Bits}: Indicates how many bits (organized in
    256-bit blocks) the device uses for feature negotiation.
...
The driver reads the device’s available features in \textbf{blocks of 256 bits}
using \msgref{GET_FEATURES}, supplying a \emph{block index} and \emph{number of
blocks} to retrieve. The device responds with the feature bits valid in that
range, possibly filling out-of-range blocks with zero.

The driver then determines which features it can or wants to enable. Enabling a
feature typically requires the driver to set the corresponding bit(s) and send
them back to the device with \msgref{SET_FEATURES}. The device, upon receiving
\textbf{SET\_FEATURES}, may reject certain bits or update bits if it cannot
enable them. The final bits returned to the driver indicate which features are
fully negotiated.

\paragraph{Error Handling}
If the driver requests blocks beyond the number of feature bits the device
exposes, the device \emph{MUST} respond with an \msgref{ERROR}. Likewise, if the
driver attempts to enable features the device does not recognize or support,
the device \emph{MAY} leave those bits unset in its final response.
...
\end{itemize}
The device returns the requested data along with the \emph{current configuration
...
\emph{MUST} reject the request with \msgref{ERROR}. The driver \emph{SHOULD}
then read the updated configuration space and reattempt if necessary.
...
\begin{enumerate}
  ...
  \item \textbf{SET\_VQUEUE}: Specify the number of descriptors (not exceeding
        ...
        and device area.  
  \item \textbf{(Optional) GET\_VQUEUE} again: Check that the configuration
...
After setting up one or more virtqueues, the driver \emph{MAY} send notifications
to signal that new buffers are available for processing. The virtio-msg transport
defines an optional message, \msgref{EVENT_AVAIL}, that the driver can use to
notify the device of pending buffers if the device’s feature negotiation allows
it (e.g., if a feature akin to \emph{NOTIFY_ON_AVAIL} is defined or if
\emph{NEGOCIATION\_DATA} was agreed upon).
...
\begin{itemize}
  ...
  \item \textbf{Response}: The \msgref{EVENT_AVAIL} message does not require a
...
If a driver chooses not to use \msgref{EVENT_AVAIL}, or if the device does not
support receiving it, the driver \emph{MAY} rely on standard virtqueue polling
mechanisms to detect completion.
...
During normal operation, the device \emph{MAY} send notifications to the driver
about configuration changes or the completion of buffers in virtqueues.

\paragraph{EVENT\_CONFIG}
\begin{itemize}
  \item Sent by the device when a configuration field changes at runtime.
...
These messages enable asynchronous updates from the device, reducing driver
polling overhead and improving performance.
...
The driver \emph{MAY} issue new \msgref{SET_CONFIG} or \msgref{SET_FEATURES}
messages after initialization if device functionality or features need to be
changed at runtime. The device \emph{MUST} respond with updated generation counts
or feature bits if the request succeeds, or \msgref{ERROR} if the request fails
(e.g., invalid generation count or unsupported features). The driver \emph{SHOULD}
be prepared to handle partial acceptance of requested features if the device
cannot enable them all simultaneously.
...
\emph{MUST} reinitialize it (see
\ref{sec:Virtio Transport Options / Virtio Over Messages / Device Initialization})
before resuming normal operation.

\paragraph{Backend-Initiated Reset}
In some circumstances, the device backend \emph{MAY} also trigger a reset if it
encounters an unrecoverable error. This can be signaled to the driver by sending
an \textbf{EVENT\_CONFIG} with a \emph{device status} indicating an error state,
or through a higher-level bus reset (\busref{STATUS} if implemented). The driver
\emph{SHOULD} treat this situation as if \msgref{SET_DEVICE_STATUS}(0) was
received and re-initialize the device accordingly.
...
A driver (frontend) typically sends these messages to a known device ID,
and the virtio-msg backend (device) responds or notifies as appropriate. The
bus \emph{forwards} these messages but does not interpret or modify them beyond
basic checks (e.g., ensuring the size does not exceed \texttt{max\_size},
verifying the target device ID exists).

Most transport messages adopt a \emph{request/response} pattern, but some are
unidirectional (e.g., asynchronous notifications). The \textbf{ERROR} message is
...
\hline
\msgref{GET_FEATURES}      & 0x3  & Driver \\
\hline
\msgref{SET_FEATURES}      & 0x4  & Driver \\
\hline
...
Certain notifications, such as \msgref{EVENT_CONFIG} or \msgref{EVENT_USED}, are
highly recommended to improve performance but \emph{may} be omitted if polling
...
This message is sent by the virtio-msg transport frontend or backend and does
...
This message is sent by the virtio-msg transport frontend and requires a
...
This message is sent by the virtio-msg backend and does not require a response.
...
This message is sent by the virtio-msg frontend and does not require a response.
...
hotplug management, global status changes, and liveness checks at the bus level.
...
This message is sent by the virtio-msg bus frontend and requires a response
from the backend.
...
This message is sent by the virtio-msg bus backend and does not require a
response.
...
This message is sent by the virtio-msg bus frontend and or backend and requires
a response.
...
The message contains a New State field which provides to the receiving the new
state the issuer is going into for which the following values are defined:
\begin{itemize}
\item 0: Reset.
\item 1: Shutdown.
\item 2: Suspend.
\item 4: Resume.
\item 5-127: Reserved.
\item 128-255: Reserved for implementation-specific usage.
\end{itemize}

This message is sent by the virtio-msg bus frontend or backend and does not
require a response.
...
\begin{itemize}
  ...
  \item An implementation \textbf{MUST} adhere to the
    \textbf{revision} and \textbf{maximum message size} constraints indicated by
    each bus instance. For example, it \textbf{MUST NOT} exceed the
    \texttt{max\_size} in any message.
\end{itemize}

\paragraph{Device-Side Requirements (Backend)}
\begin{itemize}
...
\paragraph{Driver-Side Requirements (Frontend)}
\begin{itemize}
  \item A driver \textbf{MUST} send valid requests that do not exceed
    \texttt{max\_size}, including correct offsets and block counts in
    \msgref{GET_FEATURES}, \msgref{SET_FEATURES}, \msgref{GET_CONFIG}, and
    \msgref{SET_CONFIG}.
  \item A driver \textbf{MUST} handle \msgref{ERROR} responses gracefully,
    taking appropriate recovery actions (e.g., re-reading configuration, resetting
    the device, or ceasing operations).
  \item A driver \textbf{MUST} provide the correct configuration generation count
    in \msgref{SET_CONFIG}, or else it \emph{MUST} handle an \msgref{ERROR}.
  \item A driver \textbf{MUST} initialize each device’s virtqueues (where needed)
    ...
    query the device’s status or configuration if an unexpected \msgref{EVENT_CONFIG}
    or error arises.
\end{itemize}
...
\begin{itemize}
  \item \busref{STATUS} for bus-wide resets and state changes is \emph{MAY}
    implement. Some systems handle resets through firmware or external
    mechanisms. If implemented, the bus \textbf{MUST} conform to the message
    layout.
...
\begin{itemize}
  \item It forwards \textbf{transport messages} to the appropriate device backend
    without altering or discarding valid requests.
  \item It enforces the \texttt{max\_size} and \texttt{revision} constraints
    advertised for its instance.
...
\begin{itemize}
  ...
  \item It respects the bus revision and \texttt{max\_size} limits for each bus
    instance.
...
A \emph{device} (backend) is compliant if:
\begin{itemize}
  \item It supports the core transport messages and enforces the
    configuration generation count.
  \item It implements feature negotiation and configuration updates accurately,
    returning errors for invalid operations.
...
A higher transport \texttt{revision} or additional feature bits \emph{MAY} extend
the protocol with new messages or capabilities. However, to remain compliant
with earlier revisions, devices and drivers \emph{SHOULD} continue to honor the
mandatory messages and semantics from prior revisions. If a bus instance or
device does not support an advanced feature, it \emph{MUST} reject or ignore
those requests cleanly using \msgref{ERROR}, rather than undefined behavior.

\paragraph{Conclusion}

By adhering to these requirements, virtio-msg implementations ensure consistent
behavior across diverse bus environments (whether they use message-based
enumeration or rely on external discovery), while retaining the core Virtio
model of feature negotiation, configuration updates, and virtqueue-based I/O.
